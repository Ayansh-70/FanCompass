...
const SECTIONS = [
  { id: 'A', label: 'Section A', cx: 200, cy: 60, path: 'M 130,80 Q 130,30 200,25 Q 270,30 270,80 L 245,100 Q 200,90 155,100 Z' },
  { id: 'B', label: 'Section B', cx: 320, cy: 150, path: 'M 270,80 Q 310,60 345,100 Q 370,140 360,190 L 330,175 Q 340,140 310,105 Z' },
  { id: 'C', label: 'Section C', cx: 320, cy: 270, path: 'M 360,210 Q 370,260 345,300 Q 310,340 270,320 L 310,295 Q 340,260 330,225 Z' },
  { id: 'D', label: 'Section D', cx: 200, cy: 345, path: 'M 270,320 Q 270,370 200,375 Q 130,370 130,320 L 155,300 Q 200,310 245,300 Z' },
  { id: 'E', label: 'Section E', cx: 80, cy: 270, path: 'M 130,320 Q 90,340 55,300 Q 30,260 40,210 L 70,225 Q 60,260 90,295 Z' },
  { id: 'F', label: 'Section F', cx: 80, cy: 150, path: 'M 40,190 Q 30,140 55,100 Q 90,60 130,80 L 90,105 Q 60,140 70,175 Z' },
];
...
export function StadiumMap({ selectedSection, onSelectSection }: StadiumMapProps) {
  ...
  return (
    <div className="stadium-map-container">
      ...
      >
        {/* Stadium outer ring */}
        <ellipse cx="200" cy="200" rx="190" ry="195" 
          fill="none" 
          stroke="var(--border)" 
          strokeWidth="1.5" 
          opacity="0.4"
        ...
        {SECTIONS.map(section => (
          <g key={section.id}>
            <path
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelectSection(section.id); }}}
            />
            <text 
              x={section.cx} 
              y={section.cy} 
        ...
        <text className="pitch-label" x="200" y="200">Pitch</text>
      ...
      <div className="map-selection-info" aria-live="polite">
        {selectedSection ? (
          <span>Selected: <span className="selected-name">Section {selectedSection}</span></span>
        ) : (
      ...
      <div className="map-legend">
        <div className="legend-item"><span className="legend-dot low"></span> Low crowd</div>
        <div className="legend-item"><span className="legend-dot medium"></span> Medium</div>
        <div className="legend-item"><span className="legend-dot high"></span> High crowd</div>
      </div>
    </div>
  );
}
